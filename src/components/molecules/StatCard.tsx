...
export const StatCard: FC<StatCardProps> = ({ label, value, trend, isPositive = true, icon }) => (
  <div class="p-6 rounded-2xl bg-surface/40 border border-primary/10 hover:border-primary/40 hover:shadow-[0_0_30px_-10px_rgba(138,43,226,0.3)] transition-all duration-500 group relative overflow-hidden backdrop-blur-sm">
    ...
    <div class="flex items-start justify-between mb-4">
      <p class="text-[10px] text-gray-500 uppercase tracking-[0.2em] font-bold opacity-80">{label}</p>
      {icon && <div class="text-primary/60 group-hover:text-primary transition-colors duration-500">{icon}</div>}
    </div>
